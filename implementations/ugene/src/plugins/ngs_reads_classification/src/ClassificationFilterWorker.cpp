...
bool ClassificationFilterValidator::validateTaxaListAttribute(const Actor *actor, NotificationsList &notificationList) const {
    ...
    if (!saveUnspecificSequences && taxons.isEmpty()) {
        notificationList << WorkflowNotification(tr("Set \"%1\" to \"True\" or select a taxon in \"%2\".")
                                                     ...
                                                     .arg(actor->getParameter(ClassificationFilterWorkerFactory::TAXONS)->getDisplayName()),
                                                 actor->getId());
        return false;
    }
...
void ClassificationFilterWorkerFactory::init() {
    Descriptor desc(ACTOR_ID, ClassificationFilterWorker::tr("Filter by Classification"), ClassificationFilterWorker::tr("The filter takes files with NGS reads or contigs, classified by one of the tools: "
    ...
    {
        Descriptor inD(INPUT_PORT, ClassificationFilterWorker::tr("Input sequences and tax IDs"), ClassificationFilterWorker::tr("The following input should be provided: <ul>"
        ...
        Descriptor outD(OUTPUT_PORT, ClassificationFilterWorker::tr("Output File(s)"), ClassificationFilterWorker::tr("The port outputs URLs to files with NGS reads, classified by taxon IDs: one file per each specified taxon ID per each input file (or pair of files in case of PE reads).\n\n"
    ...
    QList<Attribute *> a;
    {
        Descriptor saveUnspecificSequencesDescription(SAVE_UNSPECIFIC_SEQUENCES_ATTR_ID, ClassificationFilterWorker::tr("Save unspecific sequences"), ClassificationFilterWorker::tr("Select \"True\" to put all unspecific input sequences (i. e. sequences with tax ID = 0) into a separate file.<br>"
        ...
        Descriptor sequencingReadsDesc(SEQUENCING_READS, ClassificationFilterWorker::tr("Input data"), ClassificationFilterWorker::tr("To filter single-end (SE) reads or contigs, received by reads de novo assembly, set this parameter to \"SE reads or contigs\". Use the \"Input URL 1\" slot of the input port.<br><br>"
        ...
        Descriptor saveSequencesWithTaxidDescription(TAXONS, ClassificationFilterWorker::tr("Save sequences with taxID"), ClassificationFilterWorker::tr("Select a taxID to put all sequences that belong to this taxonomic group "
