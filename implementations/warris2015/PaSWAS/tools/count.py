...
if len(sys.argv)!=4:
  print("{0} [sequences.fasta] [targets.fasta] [mem on gpu in MB]".format(sys.argv[0]))
  sys.exit(-1)
...
max_seq_len    = max([len(x) for x in seqList1])
max_target_len = max([len(x) for x in seqList2])

length_sequences = int(math.ceil((max_seq_len/8.0)*8.0))
length_targets   = int(math.ceil((max_target_len/8.0)*8.0))
