...
void preallocBlockSums(unsigned int maxNumElements)
{
    ...
    CUDA_SAFE_CALL(cudaMalloc((void**) &g_scanStride,  
                                     level* sizeof(int)));         /////////////
    ...
    if (numBlocks > 1)
    {
        ...
        if (np2LastBlock)
        {
            prescan<true, true><<< 1, numThreadsLastBlock, sharedMemLastBlock >>>
                (outArray, inArray, g_scanBlockSums[level], g_scanStride, numEltsLastBlock, 
