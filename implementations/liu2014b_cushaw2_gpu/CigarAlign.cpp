...
const uint8_t CigarAlign::_alignRcOp[4] =
{ ALIGN_OP_M, ALIGN_OP_D, ALIGN_OP_I, ALIGN_OP_S };
const uint8_t CigarAlign::_alignOpName[4] =
{ 'M', 'I', 'D', 'S' };

CigarAlign::CigarAlign(Sequence& seq)
{
...
CigarAlign::CigarAlign(int32_t alignLength, int32_t start1, int32_t end1,
		int32_t start2, int32_t end2, int32_t numErrors,
		uint32_t* cigar, int32_t ncigar, int32_t alignScore, bool isfirst)
{
...
int CigarAlign::extendCigar(uint8_t* s, int32_t length)
{
	uint32_t* buffer;

	if(_start1 < 0 || _end1 >= length){
		Utils::log("BUG: some errors occurred in the banded local alignment\n");
		return 0;
	}
